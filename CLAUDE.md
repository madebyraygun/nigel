...
- Published invoices are static R2 objects at `i/{token}/index.html` and `i/{token}/invoice.pdf`, served under `public_base_url` (default `https://billing.example.com/i`); the 16-character random token is the only access control
