...
- **Invoicing:** `src/invoicing/` — accounts-receivable only, kept out of the transaction register. `invoices.rs`/`clients.rs` (data layer, `next_number` from the `next_invoice_number` metadata key starting at 1248, derived status via `refresh_status` — `void` derived from the `voided_at` column the way `sent` is derived from `published_at` — `ar_aging_detail` (the aging report: buckets, the open invoices behind them, and the outstanding total) with `ar_aging` as its buckets, `list_invoices` (every invoice with its client and paid-to-date in one query — a per-row `paid_amount` would be N+1 on a screen that redraws) and `payments` (one invoice's history, oldest first), `update_client`/`client_summary` behind `nigel client edit`/`client show`, and `update_invoice`/`void_invoice` guarded by `ensure_editable`/`ensure_voidable`; both update structs use the `Option` leaves-alone / `Option<Option<_>>` clears convention `rules::RuleUpdate` established), `gateway.rs` (`PaymentGateway`/`AssetPublisher`/`Mailer` traits, implemented by `stripe.rs`, `r2.rs`, `mailgun.rs` and faked in tests so nothing hits the network), `render_html.rs` + `templates/invoice.html` (single-pass `{{KEY}}` expansion over a fixed placeholder vocabulary (`PLACEHOLDERS`); `PayButton` renders a live link, an inert placeholder, or nothing; `load_template`/`template_path` resolve `<data_dir>/templates/invoice.html` over the embedded `DEFAULT_TEMPLATE`, validating at load; `Branding` carries template, company name and contact address in from the CLI), `render.rs` (`render_invoice` — the one seam turning an invoice into the HTML+PDF pair `send` publishes and `preview` writes locally; `pdf: None` without the feature), `send.rs` (Stripe link → render → R2 publish → Mailgun email → mark published; any failure leaves the invoice a draft), `sync.rs` (pull-based Stripe reconciliation, deduplicated by checkout session ID), `import_invoiceshelf.rs` (one-time InvoiceShelf migration, cents → dollars). CLI surfaces in `cli/client.rs` and `cli/invoice.rs`; usage doc in `docs/invoicing.md`
...
- Invoice status is derived, never set by hand: `refresh_status` recomputes draft/sent/partial/paid/overdue from `published_at`, the payment total, and the due date, and `void` from `voided_at` — `void_invoice` writes that timestamp and lets the same function derive the status, so no code path sets `status` directly. `void` is terminal and blocks send, pay, and edit
