...
- The books profile only matters at seed time and on the surfaces that offer the K-1 worksheet: personal books drop K-1 from the dashboard report/export pickers (`report_picker_items`/`canonical_report_idx`), from `report all` bulk exports, and from the SPA report directory, and relabel "Business Name" to "Household Name" (TUI settings, SPA settings, onboarding). The K-1 command, route, and export stay reachable by name on any profile. `/api/status` reports `profile`, defaulting to `business` while locked
