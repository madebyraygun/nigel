...
- Payment instructions are one configurable block (`payment_instructions`), rendered under the foot rule on **both** documents or on neither — `document::payment_lines` decides the lines and both renderers draw them, so the page and the PDF cannot disagree about how to pay. Nothing about bank transfers is hardcoded: an installation that takes none prints no heading and no block. `{{CONTACT}}` keeps its exact meaning for templates that use it and simply leaves the stock documents
