...
- The letterhead logo is published as **one** object for the installation, at `i/logo.png` or `i/logo.jpg` (`r2::logo_object`/`logo_key`, keyed on the image's MIME) — beside the token directories rather than inside one, because it is the operator's own mark, carries no client data, and a per-token copy would write the same bytes again for every invoice ever sent. `invoicing::logo` owns the once-per-content rule: `AssetPublisher::logo_url` answers where the object is addressed without uploading, the fingerprint of the bytes and that URL are recorded together in the `published_logo` metadata key, and `publish_letterhead_logo` uploads only when the pair differs — so an unchanged logo costs nothing and a `public_base_url` repointed at another bucket re-uploads the same image, since a stale address is as wrong as a stale picture. Only a logo `render::usable_logo` passes is ever uploaded, so nothing reaches the bucket that both documents would refuse to draw. A failed upload is a **warning, never a failed send or a failed republish**: `HostedLogo.url` stays `None`, that render falls back to the inline `data:` URI (the page stays self-contained; it simply stops rendering in Gmail), and the sentence travels as data the way republish warnings do — `SendOutcome.warnings`, `RepublishOutcome::warnings()`, `warnings` on the send response. A void's replacement notice keeps the letterhead by reading `logo::published_logo_url` back out of that metadata key rather than deriving one from configuration, so `voided_page_html` still needs no settings and can only point at an object a send actually put there
  ...
  packages/
    ...
    ui/                 # @nigel/ui — wc-* components + preview harness
      ...
      preview/          # Glob manifest, query-string router, axe suite (port 9090),
                        #   css-geometry.ts (stylesheet -> viewport pixels, for layout assertions)
  apps/
