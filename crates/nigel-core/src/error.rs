...
pub enum BlockReason {
    ...
    NotDeletable,
    /// A recurring schedule generated this invoice, and the run row recording
    /// which period it billed points at it. The schedule's id, not a count.
    FromSchedule(i64),
}
...
impl DeleteBlock {
    ...
    pub fn from_schedule(subject: &'static str, schedule_id: i64) -> Self {
        Self {
            subject,
            reason: BlockReason::FromSchedule(schedule_id),
        }
    }
...
impl fmt::Display for DeleteBlock {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self.reason {
            ...
            BlockReason::FromSchedule(schedule_id) => {
                write!(
                    f,
                    "Cannot delete: {subject} was generated by schedule {schedule_id}, which keeps it as the record of what that period billed — void it instead"
                )
            }
        }
    }
...
mod tests {
    ...
    fn delete_blocks_read_exactly_as_they_always_have() {
        let cases = [
            ...
            (
                DeleteBlock::from_schedule("invoice", 3),
                "Cannot delete: invoice was generated by schedule 3, which keeps it as the record of what that period billed — void it instead",
            ),
        ];
    ...
    fn block_reasons_have_stable_wire_codes() {
        ...
        assert_eq!(
            DeleteBlock::from_schedule("invoice", 3).reason_code(),
            "from_schedule"
        );
    }
