...
- `account_type` picks the root (`Assets:` / `Liabilities:`); `category_type` picks the other side (`Income:` / `Expenses:` / `Equity:`).
...
- [ ] #2 account_type picks the Assets:/Liabilities: root, category_type picks Income:/Expenses:/Equity:, and the existing sign convention exports the account leg verbatim with the category leg negated
- [ ] #3 Rows in reconciliations export as Beancount balance assertions, so a drifted book refuses to load
- [ ] #4 Two exports of unchanged books are byte-identical, and uncategorized transactions export against an explicit placeholder account rather than being dropped
- [ ] #5 A fixture test compares the export against a known-good file
