...
Ingest. A document arrives as a finished PDF and is filed against a client:
...
- Basic surface: `nigel document list` (status/kind/client filters), `nigel document show <id>` (detail including file, status, dates and the acceptance record), `nigel document kinds` to list/add/deactivate kinds.
...
- PDF-only in v1 by design: it is what drafting skills produce and what send attaches. The file is validated as a PDF by magic bytes, not extension.

This command is the skills contract's core verb — a business-specific skill renders a PDF with whatever tools it owns and files it with one command. Nothing here knows or cares what produced the file.
...
- [ ] #4 list, show and kinds print through pure format functions, with fictional-cast fixtures
