...
- Upload via `wc-dropzone` (a PDF variant of the import screen's lazy-upload flow), filing against a client and kind.
- Detail with the published page / PDF previewed through `wc-document-frame` (which already owns every iframe and the sandbox constant), collapsed by default like the invoice preview.
- Send in a dialog that survives its own request and shows the step trace (`wc-send-dialog` precedent, `wa-hide` prevented mid-flight).
- Accept and decline as confirmed actions collecting the recorded fields (name, date, method); the acceptance record rendered on the detail.
...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A document can be filed, sent, and accepted or declined entirely in the browser, with the acceptance record shown on the detail
