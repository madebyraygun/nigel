...
5. The ledger proper, in dependency order: **TASK-9.3** (chart merge) → **TASK-9.4** (journal schema) → **TASK-9.6** (backfill) → **TASK-9.7** (invoicing posting rules) → **TASK-9.8** (reports read lines) → **TASK-9.9** (splits), **TASK-9.10** (transfers as one entry), **TASK-9.11** (trial balance).
