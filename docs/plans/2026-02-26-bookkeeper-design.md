# Bluepeak Bookkeeper — Design Document

## Overview

A Python CLI tool (`bookkeeper`) replacing QuickBooks for Bluepeak. Cash-basis, single-entry accounting with bank CSV imports, rules-based categorization, and SQLite storage. No AI API calls — Claude Code serves as the conversational interface when needed.
