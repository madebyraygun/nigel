...
The full manual for the `backlog` CLI, as the tool itself documents it. CLAUDE.md keeps only the project-specific rules; everything else lives here, and `backlog --help` is always current.

Note: the `backlog` tool writes this manual into CLAUDE.md itself. If it reappears there after a tool upgrade, move it back here rather than letting it settle.
