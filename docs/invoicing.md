...
| `mailgun_domain` | `NIGEL_MAILGUN_DOMAIN` | `send` | `example.com` |
| `from_email` | `NIGEL_FROM_EMAIL` | `send` | `billing@example.com` |
...
| `public_base_url` | `NIGEL_PUBLIC_BASE_URL` | `send` | `https://billing.example.com/i` |
...
The two sides meet at Cloudflare: the bucket is exposed at `billing.example.com`
(a custom domain on the bucket, or an equivalent route into it), so an object
stored at key `i/{token}/index.html` is served at
`https://billing.example.com/i/{token}/`. Keep the `i/` prefix in `public_base_url`
