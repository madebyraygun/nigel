...
- [ ] **Step 2 — run, verify failure.**
- [ ] **Step 3 — implement.** Add `"COMPANY"` to `PLACEHOLDERS` and `("COMPANY", &esc(branding.company))` to the vars table. In `send.rs` replace the hardcoded `format!("Invoice #{} from Bluepeak", …)` with a company-aware subject. In `cli::invoice::send`, resolve `db::get_metadata(&conn, "company_name").unwrap_or_default()` into `Branding.company`.
- [ ] **Step 4 — verify.** Full test run.

**Verify:** the string `Bluepeak` no longer appears in `src/` — `rg -n 'Bluepeak' src/` should return nothing outside comments.
