...
**Goal:** Add first-class invoicing to Nigel — author invoices, publish PDF + static HTML to Cloudflare R2 under `billing.example.com/i/{token}/`, email clients via Mailgun, offer a Stripe Payment Link, and reconcile payments by pull-sync — so Nigel can replace InvoiceShelf.
...
```rust
    ...
    fn invoicing_config_prefers_env_over_settings() {
        ...
        assert_eq!(cfg2.public_base_url, "https://billing.example.com/i");
    }
```
...
pub fn invoicing_config_from(s: &Settings) -> InvoicingConfig {
    InvoicingConfig {
        ...
        mailgun_domain: env_or("NIGEL_MAILGUN_DOMAIN", &s.mailgun_domain).unwrap_or_else(|| "example.com".into()),
        from_email: env_or("NIGEL_FROM_EMAIL", &s.from_email).unwrap_or_else(|| "billing@example.com".into()),
        ...
        public_base_url: env_or("NIGEL_PUBLIC_BASE_URL", &s.public_base_url)
            .unwrap_or_else(|| "https://billing.example.com/i".into()),
    }
}
...
<p>To pay by bank transfer, reference invoice <strong>#{{NUMBER}}</strong>. Contact billing@example.com for account details.</p>
</body></html>
```
...
mod tests {
    ...
    fn message_fields_include_from_to_subject_html() {
        let f = message_fields("billing@example.com", "a@b.test", "Invoice #1248", "<p>hi</p>");
        assert!(f.contains(&("from".into(), "billing@example.com".into())));
...
pub fn send_invoice<G: PaymentGateway, P: AssetPublisher, M: Mailer>(
    ...
    let subject = format!("Invoice #{} from Bluepeak", invoice.number);
...
- `billing.example.com/i/{token}/` routing → `public_base_url` default (Task 6), object-key `i/{token}/…` (Task 11), doc (Task 17). ✓
