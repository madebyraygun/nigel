...
Cloudflare R2 bucket served under `billing.example.com/i/{token}/`, and emails the client via
...
```
   ...
   │ pdf.rs   (PDF render)        │──► R2 bucket ──► billing.example.com/i/{token}/
...
- **`src/mailgun.rs`** (new) — emails the invoice link + PDF from `billing@example.com` via the US
...
- Cloudflare routes `billing.example.com/i/*` to the R2 bucket (custom-domain binding or a route),
